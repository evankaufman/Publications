...
%Reviewer 4 argues that the M-JPDAF may not \emph{always} be better than the JPDAF because small covariances are not always desired. 
...
The probability of association is solved directly in [3], though this is considered too computationally expensive. Thus, the authors choose (11) adopted from [1] instead. Since the focus of this manuscript is on \emph{how} to effectively incorporate measurements and associations into the recursive filter, exactly how those measurements are \emph{associated} is beyond the scope of the paper. That is why the authors did not focus on tuning $B$ in the results. The paper is edited as follows to address the reviewer concerns:

\begin{correction}
Then, a close approximation of the probability that the $i$-th system is associated with the $j$-th measurement is adopted from [1] as
\begin{align*}
\label{eqn:betaDef}
...
When these are prevalent, a fixed value of $B$ in (11) was found from simulations by tuning to provide a close approximation of $\beta_{ij}$.
\end{correction}




\item {\itshape In Eq. (39), are $r_{i,k}$ and $\hat{r}_{i,k}$ scalars or vectors? They seem to be scalars in the first equation but vectors in the second equation. The position error should be $\\\sqrt{(x_1-\hat{x}_1)^2 + (x_2-\hat{x}_2)^2 + (x_3-\hat{x}_3)^2}$ (subscripts $i$ and $k$ dropped); it should not be the distance error. The position RMS errors should be included in all the tables; the angular errors only provide partial information. The accuracy of an algorithm should be based on the steady-state errors. A different metric may be used to characterize the behavior of the algorithm during the transitional phase. The position RMS errors should exclude the transitional phase, especially in Scenario I, where a large initial covariance is used. To avoid random variations, it is suggested to use Monte Carlo results in all the tables.}

Thanks for the comment. The expression in (39) has been revised as follows.
...
%In the presented scenarios of numerical examples, the position error is decomposed into the estimation error in the range, and the estimation error in the direction. This is motivated by the fact that there exists optimal sensors that can measure angles directly, but the range should be determined indirectly through the coupling between the range dynamics and the direction dynamics of the orbit. As such, it is naturally expected that the estimation errors in the direction remain below the level of sensor noise, and the performance of the estimator is measured directly via the error in the estimate of the range. 
%The following is added to the paper for clarification:
...
However in the GEO cases, telescope data only provide angles. The angular RMS error, $\mathbf{e}_{RMS,\theta,i}$, is tabulated because if the data association is performed accurately, properly associated measurements would have large effects on the directions of those objects. Furthermore, the GEO objects have large and nearly equal altitude, so the position RMS errors do not provide much information.
%In the presented scenarios of numerical examples, the position error is decomposed into the estimation error in the range, and the estimation error in the direction. This is motivated by the existing sensors available in the various cases. With LEO cases, a phased-array radar would provide range. With GEO cases, optical sensors provide angles-only measurements.
The following is added to the paper for clarification:
\begin{correction}
...
\end{correction}
%The authors would also like to point out that this kind of error analysis is common in the presented problem of angles-only relative orbit determination. 

%The angular RMS errors are more suitable for the considered scenarios, where the important, particularly in the angles-only GEO cases. One would expect that the direction of the objects would be very accurate, even if the positions were not.
