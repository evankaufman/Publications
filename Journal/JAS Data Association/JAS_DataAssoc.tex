...
In short, the main contributions of this paper are (i) deriving the optimal JPDAF that minimizes the trace of the posterior state covariance matrix, and (ii) proposing a systematic approach to avoid data coalescence of multiple objects in close-proximity. The desirable features of the proposed approaches are illustrated by extensive numerical simulations with satellites in orbits.
...
%The Gaussian states are tracked with measurements; however, 
...
%The $i$-th a priori innovation covariance matrix 
...
When these are prevalent, a fixed value of $B$ in \refeqn{betaDef} was found from simulations by tuning to provide a close approximation of $\beta_{ij}$.% in various applications.
...
Because \refeqn{CovExpanded} is based on the Joseph form, this equation yields $P^+_{i}$ for arbitrary gain $K_i$.
...
%The similarity between multiple multivariate Gaussian distributions can be represented by the overlapping regions of their probability density functions. Since there exists no analytic solution for this overlap, we choose Matusita's measure because it provides a scalar approximation of the overlap of multivariate Gaussian distributions~\cite{Mat55},~\cite{Coal_k}. 
We choose Matusita's measure because it provides a degree of similarity among multivariate Gaussian distributions~\cite{Mat55},~\cite{Coal_k}. 
...
Filter performance is largely governed by tuning the weighted costs, and several numerical scenarios demonstrate the coalescence-avoiding benefits of the C-JPDAF in the subsequent section.%Therefore, if the number of estimates exhibiting significant similarity is $n_t^*\leq n_t$ and $\mathcal C$ is the number of times \refeqn{NCO} is solved depending on the numerical accuracy, then the required iterations is $n_t^*(n_t^*-1)\mathcal C$, where some variables may be reused (e.g. $S^+_{pq}\equiv S^+_{qp}$) and often the effect of \refeqn{derivu} is negligible with large estimate separations.
...
In the following scenarios, several cases are simulated with \refeqn{NonLin2BP} and measurements are generated according to \refeqn{zj}, but the measurement origins are not given to the data association algorithm.
...
Since this probability is relatively small, the association probabilities are computed with \refeqn{betaDef} using $B=0$.
...
\begin{align}
% MATLAB: RMS_r_indiv(k) = (sum((Ri{k}-Rhati{k}).^2)/NumStates).^0.5;
...
\paragraph*{Scenario I: Comparison of the JPDAF and M-JPDAF with two LEO satellites in close proximity}\ 
...
\end{center}
\end{table}
In these cases, the estimates have initial covariances matrix $P_{0,\text{I}}=10^7\times Q$, and time steps are $1$ second over $100$ seconds.
...
\begin{figure}
{
%\hspace*{0.05\colu0.33mnwidth}
\centerline{
	\subfigure[\;Case I: JPDAF]
		{\hspace*{0.\columnwidth}\includegraphics[width=0.33\columnwidth]{A1JPDAF.pdf}}
	\subfigure[\;Case I.b: JPDAF]
		{\hspace*{0.\columnwidth}\includegraphics[width=0.33\columnwidth]{A1bJPDAF.pdf}}
	\subfigure[\;Case I.d: JPDAF]
		{\hspace*{0.\columnwidth}\includegraphics[width=0.33\columnwidth]{A1dJPDAF.pdf}}
	}
\centerline{
	\subfigure[\;Case I: M-JPDAF]
		{\hspace*{0.\columnwidth}\includegraphics[width=0.33\columnwidth]{A1MUJPDAF.pdf}}
	\subfigure[\;Case I.b: M-JPDAF]
		{\hspace*{0.\columnwidth}\includegraphics[width=0.33\columnwidth]{A1bMUJPDAF.pdf}}
	\subfigure[\;Case I.d: M-JPDAF]
		{\hspace*{0.\columnwidth}\includegraphics[width=0.33\columnwidth]{A1dMUJPDAF.pdf}}
	}
}
...
\end{center}
\end{table}
The initial state covariance is chosen as $P_{0,\text{II}}=Q$ for time steps of $10$ seconds over a $2000$ second time period.
...
Among the variations of the II cases, the range-rate measurement uncertainty (Case II.f) has the largest effect on tracking with the C-JPDAF.
...
Angles-only measurements are available with higher accuracy; namely the topocentric right ascension and declination are chosen as $\sigma_{\alpha}=\sigma_{\delta}=(\frac{0.001\pi}{180}\ \text{rad})^2$ and the $j$-th measurement $z_{j}\in\begin{bmatrix}\alpha, & \delta\end{bmatrix}^T\in\Re^2$ with covariance $R=\diag[\sigma_{\alpha}^2, \sigma_{\delta}^2]\in\Re^{2\times2}$.% YOU MAY HAVE TO REMOVE "\\" WITH ANY EDITING CHANGES
...
\paragraph*{Scenario III: Comparison of the JPDAF and M-JPDAF with two near-GEO satellites in close proximity}\ 
...
\end{center}
\end{table}
The III cases consider small initial covariance matrices $P_{0,\text{III}}= Q$ where $Q$ is defined the same as the LEO cases for time steps of $10$ seconds over $5000$ seconds.
...
\centerline{
	\subfigure[\;Case III: JPDAF]
		{\hspace*{0.\columnwidth}\includegraphics[width=0.33\columnwidth]{A2JPDAF.pdf}}
	\subfigure[\;Case III.a: JPDAF]
		{\hspace*{0.\columnwidth}\includegraphics[width=0.33\columnwidth]{A2aJPDAF.pdf}}
	\subfigure[\;Case III.b: JPDAF]
		{\hspace*{0.\columnwidth}\includegraphics[width=0.33\columnwidth]{A2bJPDAF.pdf}}
	}
\centerline{
	\subfigure[\;Case III: M-JPDAF]
		{\hspace*{0.\columnwidth}\includegraphics[width=0.33\columnwidth]{A2MUJPDAF.pdf}}
	\subfigure[\;Case III.a: M-JPDAF]
		{\hspace*{0.\columnwidth}\includegraphics[width=0.33\columnwidth]{A2aMUJPDAF.pdf}}
	\subfigure[\;Case III.b: M-JPDAF]
		{\hspace*{0.\columnwidth}\includegraphics[width=0.33\columnwidth]{A2bMUJPDAF.pdf}}
	}
}
