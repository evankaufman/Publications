...
% JINT17: However, there are certain issues with the current SLAM techniques. First, the vast majority of work deals only with the aspect of estimating the environment and the poses of vehicles. They are passive in the sense that SLAM is per- formed on incoming sensor measurements from vehicles following an arbitrary path. As such, human teleopera- tion and monitoring are often essential to guide the vehi- cles safely through unknown surroundings. Therefore, it is desired that an active motion planning scheme is devel- oped and integrated with SLAM such that vehicles are able to determine their path without human intervention, and explore unknown areas autonomously.
