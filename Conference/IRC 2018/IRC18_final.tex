...
% asdf Author Comments:

%----------------------- REVIEW 1 ---------------------
%PAPER: 45
%TITLE: Autonomous Quadrotor 3D Mapping and Exploration Using Exact Occupancy Probabilities
%AUTHORS: Evan Kaufman, Kuya Takami, Zhuming Ai and Taeyoung Lee
%
%Overall evaluation: 2 (accept)
%
%----------- Overall evaluation -----------
%Paper is well defined and explained. The literature review is also satisfactory. It could be better, if you give further information about how different sensors (Hokuyo, Xtion) fused together to accomplish the mapping task.
%
%
%----------------------- REVIEW 2 ---------------------
%PAPER: 45
...
%
%----------- Overall evaluation -----------
%In this paper, the authors present a framework for autonomously exploring and mappping an unknown environment from a Quadrotor. The vehicle exploits an inverse sensor model for assigning a probability (that the cell has been explored) to each cell of the map. It also considers both the entropy information of the map and the collision probabilities when computing the next optimal exploration pose.
%
%The paper is overall undestandable, but  some technical sections: 2.a, 2.b, 3.b, 3.c are not easily readable. Providing some explanations to the formule would help the reader to the interpretation.
%
%Some other comments:
%1) Passing from a full 3D map to an "augmented" 2D map is an interesting and smart trick for reducing the computational cost. In spite of this: "Typically,
%computers small enough to be carried onboard flying robots
%lack the memory or processing capabilities to update massive
%maps quickly. Therefore, an onboard computer can simply
%stream the sensor data, and a more powerful computer can
%run the mapping."  The authors should provide some references or giving more details. The computational time depends on several variables, and this seems to be a simplistic motivation for explaining the presence of a ground station for the map management.
%
%2) The ROS message filtering tool just provides in the same callback the temporally closer measurements, but it does not solve the synchronization problem. The authors probably need some interpolation method or something similar. Please explain.
%
%3) "Since most quadrotor flight is close to hover and floor plans
%exhibit little variation in the vertical direction, the proposed
%exploration uses 2D occupancy grid maps located at the
%desired exploration height." Same as (1). It depends on the context. Just by using this assumption, the proposed approach can work in some controlled scenarios with controlled quadrotor motion.
%
%4) The authors basically exploit the two map-based approach in simulation, while the composed map approach in the real environment. Probably testing both approaches in both scenarios would be more interesting. For instance: does the two-map based approach lead to the same slow convergence also in the real case?
%
%5) Main Issue: Following the problem formulation, the inverse sensor model exploits the uncertainties coming from the sensor measurements Z and from the current poses of the robot X. In Simulation, the authors just use the pose provided by the Gazebo Ground truth, while in the real environment the authors rely upon a Vicon, which provides the pose with very little uncertainty. In both cases, it seems that the authors do not introduce any noise or uncertainty in X. In this way, the experimental section cannot prove entirely the effectiveness of the proposed approach, and the reader cannot figure out if the approach can actually handle real environments.
%
%In my opinion, the paper deals with an interesting problem, but it also presents some questionable assumptions. Moreover, the experimental sections do not entirely prove the effectiveness of the proposed approach.
...
% EditEK 2: revised the sentence from:
% "Typically, computers small enough to be carried onboard flying robots lack the memory or processing capabilities to update massive maps quickly."
% to:
% Typically, computers small enough to be carried onboard flying robots lack the processing capabilities to update massive maps quickly, particularly because the ray-by-ray updating method requires many computations in series with large measurement scans. 
...
% EditEK 3: changed "message filtering" to "the Message Filters package"

\subsection{Autonomous Exploration in 3D Space}

The goal of the proposed autonomous exploration strategy is moving to poses that maximize the information the robot expects to learn about the map, measured with entropy. To achieve this in real-time, careful consideration is placed on computational costs. In particular, the expected entropy computational order of a ray that intersects $n_r$ cells is $(n_r+1)^2$. Since most quadrotor flight is close to hover and floor plans exhibit little variation in the vertical direction, the proposed exploration uses 2D occupancy grid maps located at the desired exploration height. These assumptions correspond to rooms and hallways with non-complex geometries. This approach decreases $n_r$ and simplifies the exploration problem with 3D space. These 2D maps project important properties of the 3D map.
% Edit EK 4: Added the sentence: "These assumptions correspond to rooms and hallways with non-complex geometries."
...
\begin{figure}[!t]
	\centering
	\includegraphics[width=0.45\textwidth]{entropy_gazebo_aspect3by1.png}
...
\begin{figure}[!t]
	\centering
	\includegraphics[width=0.45\textwidth]{entropy_experiment_aspect3by1.png}
