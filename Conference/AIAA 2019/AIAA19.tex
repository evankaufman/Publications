...
\title{Entropy-Based 3D Autonomous Exploration on Mars}

\author{Evan Kaufman\footnote{Graduate Research Assistant, Department of Mechanical and Aerospace Engineering, The George Washington University, evankaufman@gwu.edu.} and Taeyoung Lee\footnote{Associate Professor, Department of Mechanical and Aerospace Engineering, The George Washington University, tylee@gwu.edu.}}
...
\begin{abstract}
In this paper, we present a novel approach to explore the 3D complex environments near the surface of Mars. We apply recent developments in probabilistic 3D occupancy grid mapping to develop an exploration strategy to maximize expected map information over a complex 3D terrain. The exploration technique predicts a measure of map uncertainty using Shannon's entropy for large 3D measurement scans to determine its future pose. Then, Dijkstra's algorithm is applied in 3D with the occupancy grid to provide collision-free waypoints to safe motion planning. These contributions are demonstrated with a simulation using a height map and image from the Viking Orbiter Visual Imaging Subsystem (VIS).
