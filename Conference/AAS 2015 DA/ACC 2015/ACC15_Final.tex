...
\author{Evan Kaufman and Taeyoung Lee$^*$% Thomas Alan Lovell,
...
{
\centerline{
	\subfigure[\;Case A1: orbital radius (JPDAF), orbital radius (MUJPDAF), and uncertainty costs for both]
		{\includegraphics[width=1\columnwidth]{CaseA1_3by1.pdf}}
	}
}
{
\centerline{
	\subfigure[\;Case A2: orbital radius (JPDAF), orbital radius (MUJPDAF), and uncertainty costs for both]
		{\includegraphics[width=1\columnwidth]{CaseA2_3by1.pdf}}
	}
}
\caption{The orbital radii and uncertainty costs are compared among the algorithms (JPDAF: dashed, MUJPDAF: solid, true: solid and thick).
...
Both the JPDAF and the MUJPDAF experience coalescence due to measurement sharing; however, the coalescence is less severe with the MUJPDAF, which is consistent with the conclusions of Scenario A.
Measurements are shared with the C-JPDAF as well, but the filter coalescence minimization prevents measurement sharing from causing estimates to coalesce.
